...
app.get('/contato', (req, res) => {
	...
	res.setHeader('Access-Control-Allow-Origin', '*');
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	var sql = 'SELECT * FROM tb_contato ORDER BY id_contato COLLATE NOCASE';
		db.all(sql, [],  (err, rows ) => {
			if (err) {
				throw err;
			}
...
app.get('/competencia', (req, res) => {
	...
	res.setHeader('Access-Control-Allow-Origin', '*');
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	var sql = 'SELECT * FROM tb_info_competencia ORDER BY id_info_competencia NOCASE';
		db.all(sql, [],  (err, rows ) => {
			if (err) {
				throw err;
			}
...
app.get('/experiencia', (req, res) => {
	...
	res.setHeader('Access-Control-Allow-Origin', '*');
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	var sql = 'SELECT * FROM tb_info_experiencia ORDER BY id_info_experiencia NOCASE';
		db.all(sql, [],  (err, rows ) => {
			if (err) {
				throw err;
			}
...
app.get('/formacao', (req, res) => {
	...
	res.setHeader('Access-Control-Allow-Origin', '*');
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	var sql = 'SELECT * FROM tb_info_formacao ORDER BY id_info_formacao NOCASE';
		db.all(sql, [],  (err, rows ) => {
			if (err) {
				throw err;
			}
